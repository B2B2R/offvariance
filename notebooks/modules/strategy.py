...
def filter_total_under_games(df: DataFrame) -> DataFrame:
    return filter_games_segment(
        ...
        (
            (
                (df['odds_open_win1'] >= 1.5) & (df['odds_open_win1'] <= 8) &
                (df['odds_open_tm25'] >= 2.0) & (df['odds_open_tm25'] <= 2.5) &
                (df['expected_total'] <= 2.93) & (df['expected_total_loc'] <= 2.91)
            )
        ),
        'odds_open_tm25',
        0.05
...
def filter_away_win_games(df: DataFrame) -> DataFrame:
    return pd.concat([
        filter_games_segment(
            ...
            (
                (
                        (df['odds_open_win2'] >= 1.8) & (df['odds_open_win2'] <= 7) &
                        (df['odds_open_tb25'] >= 2.0) & (df['odds_open_tb25'] <= 2.5) &
                        (df['expected_difference'] <= -0.07) & (df['expected_difference_loc'] <= -0.07)
                )
            ),
            'odds_open_win2',
            0.05
        ...
        filter_games_segment(
            ...
            (
                (
                        ...
                        (df['expected_difference_loc'] <= -0.02) & (df['expected_difference'] <= -0.01)
                )
            ),
            'odds_open_win2',
            0.07
